...
import type { ExportSummaryCard } from "@/types/surveys";
import { Check, Copy } from "lucide-react";

export interface ExportSummaryCardsProps {
  cards: ExportSummaryCard[];
}

export function ExportSummaryCards({ cards }: ExportSummaryCardsProps) {
  const [copiedId, setCopiedId] = useState<string | null>(null);

  const copyCard = async (card: ExportSummaryCard) => {
      ...
      {cards.map((card) => (
        <Card key={card.id} className="border-dashed bg-muted/20">
          ...
          <CardContent className="space-y-2">
            ...
            {card.footnote ? (
              <p className="text-xs text-muted-foreground italic">{card.footnote}</p>
            ) : null}
          </CardContent>
        </Card>
      ))}
